...
public:
    ...
    {
        g.fillAll(palette.numBoxBackground);

        g.setColour(palette.numBoxBorder);
        g.drawRect(getLocalBounds(), 1.0f);

        g.setColour(palette.visWaveform);
        juce::Font customFont("Arial", 22.0f, juce::Font::bold);
        g.setFont(customFont);
        g.drawFittedText(juce::String(currentValue, 1), getLocalBounds(), juce::Justification::centred, 1);
    }

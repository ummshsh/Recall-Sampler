    ...
    recordTimeBox(palette)
{
    ...
    std::unique_ptr<juce::Drawable> snowflakeIconGray = juce::Drawable::createFromSVG(*juce::XmlDocument::parse(snowflakeSVG));
    std::unique_ptr<juce::Drawable> snowflakeIconBlue = juce::Drawable::createFromSVG(*juce::XmlDocument::parse(snowflakeSVG));

    snowflakeIconGray->replaceColour(juce::Colours::black, palette.freezeButtonOff);
    snowflakeIconBlue->replaceColour(juce::Colours::black, palette.freezeButtonOn);

    freezeButton.setImages(snowflakeIconGray.get(), nullptr, nullptr, nullptr, snowflakeIconBlue.get(), nullptr, nullptr, nullptr);
    freezeButton.setColour(juce::DrawableButton::backgroundColourId, juce::Colours::transparentBlack);
    freezeButton.setColour(juce::DrawableButton::backgroundOnColourId, juce::Colours::transparentBlack);
...
{
    g.fillAll(palette.editorBackground);
}

void NewProjectAudioProcessorEditor::resized()
{
    juce::Rectangle<int> area = getLocalBounds();
    flashbackVisualiser.setBounds(area);

    const int margin = 10;
    const int buttonSize = 35;
    const int numberBoxWidth = 60;
    const int numberBoxHeight = 30;

    freezeButton.setBounds(margin,
        margin,
        buttonSize,
        buttonSize);

    recordTimeBox.setBounds(area.getRight() - numberBoxWidth - margin,
        margin,
        numberBoxWidth,
        numberBoxHeight);
}